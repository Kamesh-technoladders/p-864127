...
export const EmploymentDetailsModal: React.FC<EmploymentDetailsModalProps> = ({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl p-0 overflow-hidden bg-white/90 backdrop-blur-lg border border-white/20 shadow-2xl">
        <DialogHeader className="p-4 bg-gradient-to-r from-[#ee9ca7] to-[#ffdde1]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4">
          ...
          <div className="space-y-4">
            <div className="bg-white/50 rounded-lg p-4 backdrop-blur-sm border border-white/20 transition-all duration-300 hover:shadow-lg">
              <h3 className="text-base font-semibold mb-3 text-gray-800">Basic Information</h3>
              <div className="space-y-3">
                <div>
                  <label className="text-xs text-gray-600 mb-0.5 block">Employee ID</label>
                  <Input
                ...
                <div>
                  <label className="text-xs text-gray-600 mb-0.5 block">Department</label>
                  <Input
                    value={formData.department}
                    onChange={(e) => setFormData({ ...formData, department: e.target.value })}
                    readOnly={!isEditing}
                    className={cn(
                      "h-7 text-xs transition-colors duration-200",
                      !isEditing && "bg-gray-50"
                    )}
                  />
                </div>
                <div>
                  <label className="text-xs text-gray-600 mb-0.5 block">Position</label>
                  <Input
          ...
          <div className="bg-white/50 rounded-lg p-4 backdrop-blur-sm border border-white/20">
            <h3 className="text-base font-semibold mb-4 text-gray-800">Career Timeline</h3>
            <div className="space-y-6">
              {formData.employmentHistory.map((event, index) => (
                <div key={index} className="relative">
                  {index !== formData.employmentHistory.length - 1 && (
                    <div className="absolute left-[13px] top-[24px] w-0.5 h-[calc(100%+8px)] bg-gradient-to-b from-purple-500 to-purple-100" />
                  )}
                  <div className="flex gap-3">
                    <div className="relative z-10 w-7 h-7 rounded-full bg-white flex items-center justify-center shadow-md">
